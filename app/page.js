...
function SH({t,children}){return<div style={{margin:"6px 0 10px"}}><div style={{fontSize:10,color:"var(--lh-text2)",fontWeight:800,letterSpacing:1.5,marginBottom:8,paddingBottom:6,borderBottom:`1px solid ${ls}`}}>{t}</div>{children}</div>;}
...
/* Login */
function Login({onLogin,cfg}){const[u,setU]=useState("");const[pw,setPw]=useState("");const[err,setErr]=useState("");const[ld,setLd]=useState(false);const go=async()=>{setErr("");setLd(true);try{if(pw&&pw===(cfg.adminPassword||ADMIN_PW)){onLogin({role:"admin",name:"Admin",emoji:"🛡️"});return}const all=await DB.getUsers();const f=all.find(x=>x.name.toLowerCase()===u.trim().toLowerCase()&&x.pin===pw);if(!f)setErr("Invalid name or PIN");else if(f.disabled)setErr("Account disabled");else{onLogin({role:"user",...f});return}}catch{setErr("Connection error")}setLd(false)};return<div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:bg,padding:16}}><div style={{width:"100%",maxWidth:350,animation:"fu .5s ease"}}><div style={{textAlign:"center",marginBottom:28}}><div className="av" style={{width:72,height:72,fontSize:32,margin:"0 auto 12px",background:bg}}>{cfg.appEmoji||"🫧"}</div><h1 style={{fontSize:26,fontWeight:900,color:"var(--lh-text)"}}>{cfg.appName}</h1><p style={{color:"var(--lh-text3)",fontSize:12}}>{cfg.tagline||"Smart laundry control"}</p></div><div className="nm" style={{padding:24}}><input value={u} onChange={e=>setU(e.target.value)} placeholder="Your name" autoFocus autoCapitalize="words" className="ni" style={{marginBottom:8}}/><input value={pw} onChange={e=>setPw(e.target.value)} type="password" inputMode="numeric" placeholder="PIN" onKeyDown={e=>e.key==="Enter"&&go()} className="ni" style={{marginBottom:6}}/>{err&&<p style={{color:"#f87171",fontSize:11,fontWeight:600,marginTop:4}}>{err}</p>}<button onClick={go} disabled={ld} className="nb nb-p" style={{width:"100%",marginTop:10,padding:"12px 0",fontSize:15,fontWeight:800,background:ld?"var(--lh-text3)":cfg.primaryColor,letterSpacing:.4}}>{ld?"...":"Sign In"}</button></div></div></div>;}
...
<div className="nm-in" style={{padding:"12px 14px",marginBottom:12}}>
  <div className="sb" style={{alignItems:"center",marginBottom:10}}>
    <div className="row" style={{gap:8}}><div><div style={{fontSize:10,color:"var(--lh-text3)",fontWeight:700,letterSpacing:.5}}>HISENSE CONNECT LIFE (POLLER)</div><div className="M" style={{fontSize:14,fontWeight:800,color:hsColor}}>{hsOk?"Connected":hsAge!==null?"Stale":"No data"}</div></div></div>
...
{(()=>{const washerOnline=hs&&hs.online===true&&hsAge!==null&&hsAge<60;const washerStale=hs&&hsAge!==null&&hsAge>=60&&hsAge<300;const washerOffline=!hs||hs.online===false||hsAge===null||hsAge>=300;const wColor=washerOnline?"#4ade80":washerStale?"#fbbf24":"#f87171";const wLabel=washerOnline?"Connected to WiFi":washerStale?"Stale / weak signal":(hs?.online===false?"Reported offline by Hisense":"No contact");return<div className="nm-in" style={{padding:"12px 14px",marginBottom:12}}>
  <div className="sb" style={{alignItems:"center",marginBottom:10}}>
    <div className="row" style={{gap:8}}><div><div style={{fontSize:10,color:"var(--lh-text3)",fontWeight:700,letterSpacing:.5}}>WASHING MACHINE — WiFi</div><div className="M" style={{fontSize:14,fontWeight:800,color:wColor}}>{wLabel}</div></div></div>
...
<div className="nm-in" style={{padding:"12px 14px",marginBottom:12}}>
  <div className="sb" style={{alignItems:"center",marginBottom:10}}>
    <div className="row" style={{gap:8}}><div><div style={{fontSize:10,color:"var(--lh-text3)",fontWeight:700,letterSpacing:.5}}>ESP32 RELAY CONTROLLER</div><div className="M" style={{fontSize:14,fontWeight:800,color:on?"#4ade80":"#f87171"}}>{on?"Online":"Offline"}</div></div></div>
...
{histF.length===0?<div style={{color:"var(--lh-text3)",fontSize:12,textAlign:"center",padding:"12px 0"}}>{hq?"No matches":"No washes yet"}</div>:histF.slice(0,50).map(h=><div key={h.id} className="sb" style={{padding:"7px 0",borderBottom:`1px solid ${ls}`,gap:8}}><div style={{flex:1,minWidth:0}}><div style={{fontSize:12,fontWeight:700,color:"var(--lh-text)"}}>{h.userName} — {h.cycleName}{h.stoppedBy&&<span style={{fontSize:9,color:h.stoppedBy==="esp32_safety"?"#fbbf24":"#60a5fa",marginLeft:4}}>({h.stoppedBy})</span>}</div><div style={{fontSize:10,color:"var(--lh-text3)"}}>{fmtDT(h.finishedAt,cfg.time24!==false)}{(h.waterLiters||h.energyKwh)&&` · ${h.waterLiters?h.waterLiters.toFixed(1)+"L":""}${h.waterLiters&&h.energyKwh?" · ":""}${h.energyKwh?h.energyKwh.toFixed(2)+"kWh":""}`}</div></div><span className="M" style={{fontSize:11,color:"var(--lh-text2)",whiteSpace:"nowrap"}}>{Math.round(h.durationMs/60000)}m</span></div>)}
...
<SH t="SECURITY">
<div style={{marginBottom:8}}><label style={{fontSize:11,color:"var(--lh-text3)",marginBottom:4,display:"block"}}>Admin PIN <span style={{color:"var(--lh-text2)"}}>blank = keep current</span></label><input type="password" inputMode="numeric" value={adPw} onChange={e=>sAdPw(e.target.value)} placeholder="New PIN" className="ni" autoComplete="new-password"/></div>
