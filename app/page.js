...
{inGrace?<div style={{textAlign:"center",marginTop:14,width:"100%"}}><div className="np" style={{color:"#fbbf24",marginBottom:6,letterSpacing:1}}>✓ WASH DONE</div><div style={{fontSize:14,color:"#8890a4",marginBottom:2,fontWeight:600}}>Powering off in</div><div className="M" style={{color:"#fbbf24",fontSize:40,marginTop:6,fontWeight:800,letterSpacing:1}}>{graceM}:{String(graceS).padStart(2,"0")}</div><div style={{fontSize:12,color:"#555b6e",marginTop:6}}>Relay will cut automatically</div></div>:effRunning?<div style={{textAlign:"center",marginTop:14,width:"100%"}}><div className="np" style={{color:busy||(!mac?.running&&useHs)?"#f87171":cfg.primaryColor,marginBottom:6}}>{!mac?.running&&useHs?"IN USE":(busy?"IN USE":"RUNNING")}</div>{mac?.running?<div style={{fontSize:20,fontWeight:700,color:"#e2e6ef"}}>{mac.cycleName}</div>:<div style={{fontSize:15,color:"#8890a4",marginBottom:2}}>Started on machine</div>}{useHs&&hs.phaseName&&hs.phaseName!=="Idle"&&!hs.paused&&<div style={{fontSize:11,color:cfg.primaryColor,fontWeight:700,marginTop:2,letterSpacing:.5,textTransform:"uppercase"}}>{hs.phaseName}</div>}<div className="M" style={{color:useHs&&hs.paused?"#f87171":"#8890a4",fontSize:36,marginTop:8,fontWeight:useHs&&hs.paused?700:500,transition:"color .2s"}}>{useHs?`${rmM}m`:`${rmM}:${String(rmS).padStart(2,"0")}`}</div>{!(useHs&&hs.paused)&&rmM>0&&<div style={{fontSize:13,color:"#555b6e",marginTop:6,fontWeight:600}}>Ends at {fmtT(useHs?(Date.now()+rmM*60000):(mac.startTime+mac.durationMs),cfg.time24!==false)}</div>}{useHs&&hs.paused&&<div style={{marginTop:10}}><span style={{fontSize:16,color:"#fff",background:"#f87171",padding:"6px 18px",borderRadius:14,fontWeight:900,letterSpacing:3,display:"inline-block",boxShadow:"0 2px 10px #f8717155"}}>⏸ PAUSED</span></div>}</div>:<div style={{textAlign:"center",marginTop:14}}><div className="np" style={{color:on?"#4ade80":"#f87171"}}>{on?"AVAILABLE":"OFFLINE"}</div>{blocked&&on&&<div style={{fontSize:10,color:"#fbbf24",marginTop:4}}>Reserved by {blocked.userName}</div>}{!on&&<div style={{fontSize:10,color:"#f87171",marginTop:4}}>Machine disconnected</div>}{on&&!blocked&&mac?.lastUser&&<div style={{fontSize:10,color:"#555b6e",marginTop:4}}>Last: {mac.lastUser}</div>}</div>}
{mac?.running&&!(useHs&&(hs.running||hs.paused))&&((Number(mac.startTime)||0)>0&&(Date.now()-(Number(mac.startTime)||0))>60000)&&<button onClick={async()=>{if(!confirm("Turn off the machine? This cuts power to the wash."))return;await DB.setMachine({running:false,lastUser:mac?.userName||user.name,lastCycle:mac?.cycleName||"",finishedAt:Date.now()});toast("Powered off","info")}} className="nb" style={{marginTop:14,padding:"12px 0",width:"100%",color:"#fbbf24",fontSize:14,fontWeight:800,letterSpacing:.5,border:`1px solid #fbbf2444`}}>🔌 Turn OFF</button>}
</div>
...
<div className="nm" style={{padding:22}}><div className="sec"><span className="sec-ico">🏠</span>Housemates</div><div className="g2" style={{gap:8}}>{users.filter(x=>!x.disabled&&x.id!==user.id).map(u=>{
