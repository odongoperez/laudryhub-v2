...
const DEF={primaryColor:"#6C9BCF",accentColor:"#E8A87C",appName:"LaundryHub",esp32Ip:"",alertMinutesBefore:5,maxWashMinutes:120,minWashMinutes:5,chatEnabled:true,appEmoji:"🫧",maintenance:false,tagline:"Smart laundry control",reduceMotion:false,defaultUserEmoji:"😊",confirmStop:true,autoLogoutMin:0,time24:true,soundOnFinish:true};
...
{inGrace?<div style={{textAlign:"center",marginTop:14,width:"100%"}}><div className="np" style={{color:"#fbbf24",marginBottom:6,letterSpacing:1}}>✓ WASH DONE</div><div style={{fontSize:14,color:"#8890a4",marginBottom:2,fontWeight:600}}>Powering off in</div><div className="M" style={{color:"#fbbf24",fontSize:40,marginTop:6,fontWeight:800,letterSpacing:1}}>{graceM}:{String(graceS).padStart(2,"0")}</div><div style={{fontSize:12,color:"#555b6e",marginTop:6}}>Relay will cut automatically</div></div>:effRunning?<div style={{textAlign:"center",marginTop:14,width:"100%"}}><div className="np" style={{color:busy||(!mac?.running&&useHs)?"#f87171":cfg.primaryColor,marginBottom:6}}>{!mac?.running&&useHs?"IN USE":(busy?"IN USE":"RUNNING")}</div>{mac?.running?<div><div style={{fontSize:20,fontWeight:700,color:"#e2e6ef"}}>{mac.cycleName}</div>{(mac.userId==="machine"||mac.userName==="Machine")&&<div style={{fontSize:11,color:cfg.primaryColor,fontWeight:700,marginTop:3,letterSpacing:.3}}>🌀 Started on machine</div>}{mac.userName&&mac.userName!=="Machine"&&mac.userId!=="machine"&&<div style={{fontSize:11,color:"#8890a4",fontWeight:600,marginTop:3}}>by {mac.userName}</div>}</div>:<div style={{fontSize:15,color:"#8890a4",marginBottom:2}}>Started on machine</div>}{useHs&&hs.phaseName&&hs.phaseName!=="Idle"&&!hs.paused&&<div style={{fontSize:11,color:cfg.primaryColor,fontWeight:700,marginTop:2,letterSpacing:.5,textTransform:"uppercase"}}>{hs.phaseName}</div>}<div className="M" style={{color:useHs&&hs.paused?"#f87171":"#8890a4",fontSize:36,marginTop:8,fontWeight:useHs&&hs.paused?700:500,transition:"color .2s"}}>{useHs?`${rmM}m`:`${rmM}:${String(rmS).padStart(2,"0")}`}</div>{!(useHs&&hs.paused)&&rmM>0&&<div style={{fontSize:13,color:"#555b6e",marginTop:6,fontWeight:600}}>Ends at {fmtT(useHs?(Date.now()+rmM*60000):(mac.startTime+mac.durationMs),cfg.time24!==false)}</div>}{useHs&&hs.paused&&<div style={{marginTop:10}}><span style={{fontSize:16,color:"#fff",background:"#f87171",padding:"6px 18px",borderRadius:14,fontWeight:900,letterSpacing:3,display:"inline-block",boxShadow:"0 2px 10px #f8717155"}}>⏸ PAUSED</span></div>}</div>:<div style={{textAlign:"center",marginTop:14}}><div className="np" style={{color:on?"#4ade80":"#f87171"}}>{on?"AVAILABLE":"OFFLINE"}</div>{blocked&&on&&<div style={{fontSize:10,color:"#fbbf24",marginTop:4}}>Reserved by {blocked.userName}</div>}{!on&&<div style={{fontSize:10,color:"#f87171",marginTop:4}}>Machine disconnected</div>}{on&&!blocked&&mac?.lastUser&&<div style={{fontSize:10,color:"#555b6e",marginTop:4}}>Last: {mac.lastUser}</div>}</div>}
...
<button onClick={start} className="nb nb-p" style={{width:"100%",padding:"16px 0",fontSize:16,fontWeight:800,background:on&&!blocked?cfg.primaryColor:"#555b6e",borderRadius:14,letterSpacing:.3}}>{!on?"⚠️ Machine Offline":blocked?`⏳ Reserved by ${blocked.userName}`:"✨ Turn ON — 1h 30m"}</button>
</div>}
...
{(()=>{const realRunning=mac?.running&&!hsSaysDone;const attributable=realRunning&&users.some(x=>!x.disabled&&(mac?.userId===x.id||mac?.userName===x.name||mac?.lastUser===x.name||mac?.lastUserId===x.id));if(realRunning&&!attributable)return<div className="nm-in" style={{padding:"10px 12px",marginBottom:10,display:"flex",alignItems:"center",gap:10,border:`1px solid ${cfg.primaryColor}33`}}><span style={{fontSize:18,animation:"spin 3s linear infinite",display:"inline-block"}}>🌀</span><div style={{flex:1}}><div style={{fontSize:12,fontWeight:800,color:cfg.primaryColor}}>Machine in use</div><div style={{fontSize:10,color:"#8890a4"}}>Started directly on the machine — wash in progress{useHs&&typeof hs.remainingMin==="number"?` · ${hs.remainingMin}m left`:""}</div></div></div>})()}
...
const graceLeft=hs&&hs.graceUntil?Math.max(0,hs.graceUntil-now):0;const inGrace=graceLeft>0;
...
<div className="nm" style={{marginBottom:14,display:"flex",alignItems:"center",gap:14}}><Wash on={mac?.running} prog={prog} c={cfg.primaryColor} sz={95}/><div style={{flex:1}}>{mac?.running?<><div className="np" style={{color:cfg.primaryColor,marginBottom:4}}>RUNNING</div><div style={{fontSize:14,fontWeight:800,color:"#e2e6ef"}}>{mac.userName}</div><div style={{fontSize:12,color:"#8890a4"}}>{mac.cycleName} · {rmM}m left</div><button onClick={()=>forceStop("Stopped","warning")} className="nb" style={{marginTop:8,fontSize:10,padding:"5px 14px",color:"#f87171"}}>Force Stop</button></>:<><div className="np" style={{color:on?"#4ade80":"#f87171"}}>{on?"IDLE":"OFFLINE"}</div>{mac?.lastUser&&<div style={{fontSize:11,color:"#555b6e",marginTop:3}}>Last: {mac.lastUser}</div>}<button onClick={()=>sSFU(true)} className="nb" style={{marginTop:8,fontSize:10,padding:"5px 14px",color:cfg.primaryColor}}>Start for user...</button></>}</div></div>
...
{tab==="ctrl"&&<div className="nm"><div className="sec"><span className="sec-ico">🎛️</span>Control</div><div className="nm-in" style={{marginBottom:14}}><div style={{fontSize:13,fontWeight:700,color:mac?.running?"#f87171":"#4ade80"}}>{mac?.running?`Running — ${mac.userName} (${mac.cycleName}) — ${rmM}m`:"Idle"}</div>{mac?.running&&<button onClick={()=>forceStop("Stopped","warning")} className="nb" style={{marginTop:8,color:"#f87171",fontSize:11}}>Force Stop</button>}</div><button onClick={()=>sSFU(true)} className="nb nb-p" style={{width:"100%",background:cfg.primaryColor,marginBottom:14}}>Start for user...</button>{mac?.running&&<button onClick={()=>{if(!confirm("🚨 EMERGENCY STOP — cut power immediately?"))return;DB.setMachine({running:false});toast("🚨 Emergency stop","error")}} className="nb" style={{width:"100%",marginBottom:14,padding:"14px 0",background:"#f87171",color:"#fff",fontWeight:900,fontSize:15,letterSpacing:1}}>🚨 EMERGENCY STOP</button>}
...
<div><label style={{fontSize:11,color:"#555b6e",marginBottom:6,display:"block"}}>Spinner style <span style={{color:"#8890a4"}}>(main wash animation)</span></label><div className="wrap">{[{k:"drop",l:"💧 Drop"},{k:"pulse",l:"● Pulse"},{k:"ring",l:"◜ Ring"},{k:"dots",l:"••• Dots"},{k:"bubble",l:"🫧 Bubble"},{k:"wave",l:"▊ Wave"}].map(s=><button key={s.k} onClick={()=>sSpinStyle(s.k)} className="nb" style={{fontSize:11,padding:"7px 12px",color:spinStyle===s.k?cfg.primaryColor:"#c8cdd8",fontWeight:spinStyle===s.k?800:600}}>{s.l}</button>)}</div><div style={{marginTop:10,display:"flex",justifyContent:"center",padding:"14px 0"}}><Wash on={true} prog={.65} c={cP} paused={false} grace={false} spinnerStyle={spinStyle} sz={90}/></div></div>
